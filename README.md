...
`Use (sharedObj)` … `End use` guards shared-object and shared-collection
access **[verified]**; blocks nest, and the parens are part of the expression
like every other condition (§4.9).
...
| `onHTTPGet` | function | Additionally callable via HTTP GET (added in 20 R7) **[verified / corpus]** |
| `local` | function | Executes on the client |
| `server` | function | Always executes on the server |

None of these are *reserved* words — they are contextual keywords, special
only in the slot between line start and `Function` / `Class constructor`.
`$onHTTPGet:=1`, a method named `singleton`, and `$o.exposed` are all legal,
and the grammar parses them as plain identifiers: tree-sitter's keyword
extraction only prefers the modifier token in parser states where a modifier
is actually valid. The scanner's `_function_start` peek mirrors this — a line
beginning with these words claims a function header only if the word run
terminates in `Function` or `Class constructor`.
